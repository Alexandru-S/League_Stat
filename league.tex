...
\end{align*}\\
As predicted and observed in the above graphs, when there is an advantage of winning at home the graphs tend to be skewed towards the higher end of the values. Similarly when there is a disadvantage the graphs are skewed towards the lower end of the values. The two graphs therefore creating almost mirror images of each other.
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{win_b.png}
\caption{Probability of Winning with Biased odds}
\end{figure}
By using the results of two binomial distributions the above graph was created. It depicts that by using a biased home and away point system, the most likely probability in a tournament is to win half of the matches.\cite{dekker} Therefore by following a home and away biased point system this negates the bias introduced by home advantage and away disadvantage in games and reintroduces the notion of fair play.
